-- 巴西 Olist 電商平台資料合併查詢（訂單層級，一單一列）
-- 與 merged_olist_data VIEW 的核心邏輯一致
...
FROM review_best rv
JOIN olist_orders_dataset o ON rv.order_id = o.order_id
JOIN olist_customers_dataset c ON o.customer_id = c.customer_id
LEFT JOIN items_agg ia ON o.order_id = ia.order_id
LEFT JOIN order_cat oc ON o.order_id = oc.order_id
LEFT JOIN items_list il ON o.order_id = il.order_id
LEFT JOIN seller_order so ON o.order_id = so.order_id
LEFT JOIN olist_sellers_dataset s ON so.primary_seller_id = s.seller_id
